...
public final class AircraftScheduler {
    ...
    public void schedule() {
        ...
        ArrayList<AircraftSchedule> finishedSchedules = new ArrayList<>();
        airline.getFleet().getFleet().stream().parallel().forEach(sf -> finishedSchedules.addAll(scheduleSubfleet(sf.getAcfType())));
        finishedSchedules.stream().parallel().forEach(fs -> setTimes(fs));
     ...
     */
    protected void setTimes(AircraftSchedule as){
        ...
        sortFlights(as);
        ArrayList<Flight> flights = as.getFlights();
     ...
     */
    protected void sortFlights(AircraftSchedule as){
        //The engine that puts the flights in order
        GenerationalEvolutionEngine<List<Flight>> engine = new GenerationalEvolutionEngine<List<Flight>>(
     ...
     */
    private ArrayList<AircraftSchedule> scheduleSubfleet(AircraftType type) {
        ...
        ArrayList<AircraftSchedule> finished = new ArrayList<>();
        //Fill out the airplanes
        airline.getFleet().getSubFleet(type.getIcao()).getAircraft().values().forEach(p -> airplanes.add(p));
        //Fill out the flights
        airline.getRoutes().stream().
                filter(new Predicate<Route>() {
                    @Override
                    public boolean test(Route route) {
                        return route.getType().equals(type);
                    }
                }).forEach(r ->
        ...
        fixSchedules(end.stream().parallel().filter(new Predicate<EvaluatedCandidate<AircraftSchedule>>() {
            @Override
            public boolean test(EvaluatedCandidate<AircraftSchedule> aircraftScheduleEvaluatedCandidate) {
                return aircraftScheduleEvaluatedCandidate.getFitness() < 10;
            }
        }));
     ...
     * @return a fixed list
     */
    protected List<AircraftSchedule> fixSchedules(Stream<EvaluatedCandidate<AircraftSchedule>> schedules){
        ArrayList<AircraftSchedule> scheds = new ArrayList<>();
    ...
    private AircraftSchedule fixSchedule(AircraftSchedule schedule){
        AircraftSchedule sched = schedule;
        while(schedule.score() < 10.0d && schedule.getFlights().size() > 0){
            Flight f = sched.removeRandom();
            Flight ret = null;
            try {
     ...
     */
    private Flight getReturn(Flight f, AircraftSchedule s) throws InvalidParameterException{
        ArrayList<Double> distances = new ArrayList<>();
        Stream<Flight> allReturns = s.getFlights().stream().filter(new Predicate<Flight>() {
            @Override
            public boolean test(Flight flight) {
                return flight.getRoute().getDeparts().equals(f.getRoute().getArrives()) && flight.getRoute().getArrives().equals(f.getRoute().getDeparts());
            }
        });
     ...
     */
    private List<AircraftSchedule> randomSchedule(List<Flight> toSchedule, List<Airplane> airplanes){
        MersenneTwister rand = new MersenneTwister();
        List<AircraftSchedule> schedules = new ArrayList<>();
        airplanes.forEach(a -> schedules.add(AircraftSchedule.emptySchedule(a)));
        toSchedule.forEach(flt -> schedules.get(rand.nextInt(schedules.size())).add(flt));
        return schedules;
    }
